...
def main():
    ...
    root = repo / ".cache" / "calibration-shaft-reuse" / uuid4().hex
    isolate(root / "state")
    import cv2

    from menipy.common import auto_calibrator as detection
    from menipy.common.auto_calibrator import run_auto_calibration
    from tests.test_calibration_shaft_reuse import (
        assert_calibration_equal,
        repeated_shaft_drop,
    )

    report = {"platform": platform.platform(), "python": sys.version, "cases": {}}
    functions = {"original": repeated_shaft_drop, "optimized": detection.detect_sessile_drop_contour}
    for mode, filename in [("sessile", "sessile_needle_reference.png"),
        ...
        for repeat in range(6):
            ...
            for name in (list(functions) if repeat % 2 == 0 else list(reversed(functions))):
                with patch.object(detection, "detect_sessile_drop_contour", functions[name]):
