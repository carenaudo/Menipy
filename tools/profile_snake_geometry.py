"""Paired full evolution timings for deferred curvature calculation."""
...
def main():
    ...
    root = Path(__file__).resolve().parents[1] / ".cache" / "snake-geometry" / uuid4().hex
    isolate(root / "state")
    from menipy.math import active_contour as snake
    from tests.test_active_contour import make_synthetic_circle_image
    from tests.test_snake_geometry_work import original_iteration_normals

    image = make_synthetic_circle_image()
    optimized = snake._evolution_normals
    ...
    for label, line, flux in (("default", 0, 0), ("combined", 0.2, 0.3)):
        for n in (80, 300):
            ...
            functions = {"original": original_iteration_normals, "shared": optimized}
            times = {name: [] for name in functions}
            for func in functions.values():
                with patch.object(snake, "_evolution_normals", func):
            ...
            for repeat in range(6):
                ...
                for name in (list(functions) if repeat % 2 == 0 else list(reversed(functions))):
                    with patch.object(snake, "_evolution_normals", functions[name]):
