...
for host_name in ("runHost", "overlayHost", "resultsHost"):
    ...
    if host is None:
        ...
        continue
    children = [
        w.objectName() for w in host.findChildren(QWidget) if w.parent() is host
    ]
...
print(
    "plugin_dock widget objectName:",
    dock_widget.objectName(),
    "type:",
    type(dock_widget),
)
print(
    "plugin_dock children:", [w.objectName() for w in dock_widget.findChildren(QWidget)]
)
...
print(
    ...
    [w.objectName() for w in cw.findChildren(QWidget) if w.parent() is cw],
)
