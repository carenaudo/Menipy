...
try:
    ...
    subprocess.check_call([sys.executable, '-m', 'PySide6.scripts.rcc', str(QRC), '-o', str(OUT)])
    print('Wrote:', OUT)
    sys.exit(0)
except Exception as e:
    print('PySide6.scripts.rcc failed:', e)

print('Could not build resource file automatically. Please install pyside6-rcc or run the rcc tool manually.')
sys.exit(2)
