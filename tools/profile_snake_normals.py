"""Paired full evolution timings for skipping unused iteration normals."""
...
def main():
    ...
    root = Path(__file__).resolve().parents[1] / ".cache" / "snake-normals" / uuid4().hex
    isolate(root / "state")
    from menipy.math import active_contour as snake
    from tests.test_active_contour import make_synthetic_circle_image
    from tests.test_snake_normal_skip import previous_normals
    ...
    for label, line, flux in (("default", 0, 0), ("combined", 0.2, 0.3)):
        for n in (80, 300):
            ...
            functions = {"original": previous_normals, "shared": optimized}
