...
all_py = [p for p in Path(repo_root).rglob('*.py') if repo_root in p.parents or p == repo_root]
for p in all_py:
    try:
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Import):
            ...
            if level == 0:
                ...
                target_parts = cur_parts[:len(cur_parts)-level+1] if level>0 else cur_parts
                if mod:
                    mod_parts = mod.split('.')
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.ImportFrom):
            if node.level >= 1:
                ...
                target_parts = base_parts[:len(base_parts)-node.level+1] if node.level>0 else base_parts
                if node.module:
