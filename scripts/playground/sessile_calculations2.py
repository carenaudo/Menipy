...
def fit_young_laplace(contour_points, contact_left, contact_right, substrate_y, 
                     rho=1000, sigma=72e-3, g=9.81):
    """
    Fit Young-Laplace equation to drop profile.
    ...
    Returns: contact angles, capillary length, Bond number
    """
    points = contour_points.copy()
    ...
    # Normalize coordinates (apex at origin)
    ...
    # Take only one side (symmetry) - right side
    right_side = points_norm[points_norm[:, 0] >= 0]
    
    if len(right_side) < 10:
        return None, None, None, None
    
    # Capillary length: sqrt(sigma / (rho * g))
    capillary_length = np.sqrt(sigma / (rho * g))
    
    # Bond number: Bo = rho * g * R² / sigma
    # Using half-width as characteristic length
    R_char = (base_width / 2) / 1000  # Convert px to meters (rough estimate)
    Bo = rho * g * R_char**2 / sigma
    
    def residual(params):
        """Calculate residual for optimization"""
        b = params[0]  # Curvature at apex
        
        z_span = right_side[:, 1]
        y0 = [0, 0]  # Initial conditions at apex: r=0, phi=0
        
        try:
            # Solve ODE
            sol = odeint(young_laplace_profile, y0, z_span, args=([b, Bo],))
            r_fit = sol[:, 0]
            ...
            return np.ones_like(right_side[:, 0]) * 1e6
    
    # Initial guess for curvature
    try:
        result = least_squares(residual, x0=[2.0], bounds=([0.1], [10.0]))
        b_opt = result.x[0]
        
        # Calculate final profile
        z_span = right_side[:, 1]
        y0 = [0, 0]
        sol = odeint(young_laplace_profile, y0, z_span, args=([b_opt, Bo],))
        ...
        return angle_left, angle_right, capillary_length, Bo
    except:
        return None, None, None, None
...
def sessile_drop_adaptive(image_path):
    ...
    angle_left_yl, angle_right_yl, cap_length, Bo = \
    ...
    if angle_left_yl is not None:
        ...
        print(f"  Capillary Length:    {cap_length*1000:.2f} mm")
        print(f"  Bond Number:         {Bo:.4f}")
    else:
    ...
    plt.tight_layout()
    # plt.show()
    output_filename = f"python_result_{os.path.basename(image_path)}.png"
