...
function debug_plots_makie(img, img_binary, img_binary_roi, droplet_mask, contour;
    ...
    ax1 = Axis(fig[1, 1], title="Original (scaled)")
    image!(ax1, img_s)
    hidedecorations!(ax1)
    hidespines!(ax1)

    ax2 = Axis(fig[1, 2], title="Binary global (scaled)")
    image!(ax2, bin_s)
    hidedecorations!(ax2)
    hidespines!(ax2)

    ax3 = Axis(fig[2, 1], title="ROI + mask overlay (scaled)")
    image!(ax3, roi_s)
    image!(ax3, mask_s, alpha=0.45)
    hidedecorations!(ax3)
    hidespines!(ax3)

    ax4 = Axis(fig[2, 2], title="Contour overlay (scaled)")
    image!(ax4, img_s)
    if !isempty(xs)
        lines!(ax4, xs, ys, linewidth=2)
    end
    hidedecorations!(ax4)
    hidespines!(ax4)
...
function extract_contour_ordered(img_binary::BitMatrix)
    ...
    # Contornos (Suzuki–Abe style border following)
    contours = find_contours(droplet_clean)

    if isempty(contours)
        return Tuple{Int,Int}[], droplet_clean
    end

    # Elegir el contorno más largo (droplet exterior típicamente)
    lengths = map(length, contours)
    C = contours[argmax(lengths)]

    # Convertir a tuplas (x,y) = (col,row)
    contour_points = [(p[2], p[1]) for p in C]

    return contour_points, droplet_clean
end

"""
    debug_plots_plots(img, img_gray, img_binary, img_binary_roi, droplet_mask, contour; save_path=nothing)
...
"""
function debug_plots_plots(img, img_gray, img_binary, img_binary_roi, droplet_mask, contour; save_path=nothing)
    # 1. Original
