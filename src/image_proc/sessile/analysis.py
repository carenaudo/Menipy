...
def analyze_sessile(image: np.ndarray, substrate_poly: np.ndarray) -> SessileResult:
    """Return sessile-drop metrics using an improved segmentation workflow."""

    cal = get_calibration().pixels_per_mm

    # --- Stage 1: crop & denoise ------------------------------------------
    gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY) if image.ndim == 3 else image
    filt = cv2.bilateralFilter(gray, 5, 25, 25)

    # --- Stage 2: build exclusive masks -----------------------------------
    _, nr_mask = cv2.threshold(filt, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
    roi_mask = cv2.adaptiveThreshold(
        filt, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 31, 2
    )
    drop_mask = cv2.bitwise_and(roi_mask, cv2.bitwise_not(nr_mask))

    # --- Stage 3: keep the physical droplet side --------------------------
    v = substrate_poly[-1] - substrate_poly[0]
    grid_y, grid_x = np.indices(drop_mask.shape)
    cross_mask = v[0] * (grid_y - substrate_poly[0, 1]) - v[1] * (
        grid_x - substrate_poly[0, 0]
    )
    contour_tmp, _ = cv2.findContours(drop_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
    contour = max(contour_tmp, key=cv2.contourArea).squeeze(1).astype(float) if contour_tmp else None
    if contour is None or contour.size == 0:
        raise ValueError("could not extract droplet contour")
    cross = np.cross(v, contour - substrate_poly[0])
    sign = 1 if np.sum(cross > 0) >= np.sum(cross <= 0) else -1
    drop_mask[cross_mask * sign < 0] = 0

    # Remove mirrored fragments touching the other side
    line_img = np.zeros_like(drop_mask)
    cv2.polylines(line_img, [np.round(substrate_poly).astype(np.int32)], False, 255, 3)
    drop_mask[cv2.dilate(line_img, np.ones((3, 3), np.uint8)) > 0] = 0

    # --- Stage 4: contour and P1/P2 ---------------------------------------
    contours, _ = cv2.findContours(drop_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
    if not contours:
        raise ValueError("droplet not detected")
    ...
    width_px = float(np.hypot(p1[0] - p2[0], p1[1] - p2[1]))

    # --- Stage 5: apex & symmetry axis ------------------------------------
    ...
    a = np.polyfit(contour[:, 0], contour[:, 1], 1)
    slope = a[0]
    intercept = apex[1] - slope * apex[0]
    y0 = 0
    y1 = drop_mask.shape[0] - 1
    ...
    # --- Stage 6: metrics --------------------------------------------------
    area_px = int(cv2.countNonZero(drop_mask))
    height_px = _distance_to_line(apex, substrate_poly[0], substrate_poly[-1])
    volume_px3 = float(area_px * height_px / 3)
