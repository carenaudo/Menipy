...
class PendantPipeline(PipelineBase):
    ...
    def do_acquisition(self, ctx: Context) -> Optional[Context]:
        ...
        self.logger.info(
            f"[do_acquisition] ctx.image type: {type(getattr(ctx, 'image', None))}, ctx.image_path: {getattr(ctx, 'image_path', None)}"
        )

        # If frames already exist, nothing to do
        if getattr(ctx, "frames", None) and len(ctx.frames) > 0:
            self.logger.info(
                f"[do_acquisition] Frames already exist: {len(ctx.frames)}"
            )
            return ctx
