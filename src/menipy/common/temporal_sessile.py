...
def _summarize(frames: list[TemporalFrameResult], fps: float) -> dict[str, Any]:
    summary: dict[str, Any] = {
        ...
        "duration_s": (
            frames[-1].timestamp_s - frames[0].timestamp_s if len(frames) > 1 else 0.0
        ),
        "fps": fps,
    }
    for state in ("advancing", "receding"):
        check_cancelled()
        selected = [
            frame for frame in frames if frame.accepted and frame.state == state
        ]
        summary[f"{state}_duration_s"] = len(selected) / fps
        velocities = [
            frame.contact_velocity_mm_s
    ...
    *,
    ...
    for frame_index, frame in enumerate(frames):
        ...
        if previous_line is not None:
            ...
            if (
                abs(line_offset - prior_offset) > 5.0
                or abs(line_angle - prior_angle) > 1.0
            ):
        ...
        if previous_contacts is not None and lost == 0:
            ...
            displacement = max(
                np.linalg.norm(
                    np.asarray(contacts[index]) - np.asarray(previous_contacts[index])
                )
                for index in (0, 1)
            )
        ...
        if previous_contour is not None and previous_contacts is not None:
            result.predicted_roi = _predicted_roi(
                previous_contour, previous_contacts, contact_velocity, frame.image.shape
            )
        ...
        result.half_width_mm = float(
            np.linalg.norm(np.asarray(contacts[1]) - np.asarray(contacts[0]))
            / (2.0 * float(px_per_mm))
        )
        ...
        output.append(result)
        previous_line, previous_contacts, previous_contour, previous_area = (
            line,
            contacts,
            contour,
            area,
    ...
    return DynamicSessileResult(
        ...
        frames=output,
        diagnostics={
            "velocity_deadband_mm_s": deadband,
            "max_invalid_run": maximum_gap,
            "raw_values_promoted": True,
            "classification_window_frames": 7,
        },
    )


def export_dynamic_result(
    result: DynamicSessileResult, output_dir: str | Path
) -> dict[str, Path]:
            ...
            writer.writerow(
                {
                    ...
                    "baseline_x1": baseline[0][0],
                    "baseline_y1": baseline[0][1],
                    "baseline_x2": baseline[1][0],
                    "baseline_y2": baseline[1][1],
                    "contact_left_x": contacts[0][0],
                    "contact_left_y": contacts[0][1],
                    "contact_right_x": contacts[1][0],
                    "contact_right_y": contacts[1][1],
