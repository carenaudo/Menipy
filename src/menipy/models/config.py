...
class FilterSettings(BaseModel):
    ...
    enabled: bool = Field(default=False)
    method: Literal["none", "gaussian", "median", "bilateral"] = Field(default="none")
    ...
    sigma_color: float = Field(
        default=75.0, ge=0.0, description="Bilateral color sigma"
    )
    sigma_space: float = Field(
        default=75.0, ge=0.0, description="Bilateral spatial sigma"
    )
...
class PreprocessingSettings(BaseModel):
    ...
    convert_to_grayscale: bool = Field(
        default=True,
        description="Convert the source image/ROI to grayscale before other steps.",
    )
    ...
    class FillHolesSettings(BaseModel):
        ...
        enabled: bool = Field(
            default=False,
            description="Enable filling small holes and removing spurious objects",
        )
        max_hole_area: int = Field(
            default=500, ge=0, description="Maximum hole area (in pixels) to fill"
        )
        remove_spurious_near_contact: bool = Field(
            default=True,
            description="Attempt to remove small objects near the contact line",
        )
        proximity_px: int = Field(
            default=5,
            ge=0,
            description="Pixel proximity to contact line to consider for spurious removal",
        )
...
class EdgeDetectionSettings(BaseModel):
    ...
    canny_aperture_size: Literal[3, 5, 7] = Field(
        default=3, description="Aperture size for the Sobel operator"
    )
    canny_L2_gradient: bool = Field(
        default=False,
        description="Flag indicating whether a more accurate L2 gradient magnitude should be used",
    )

    # Threshold specific parameters
    threshold_value: int = Field(
        default=128, ge=0, le=255, description="Threshold value"
    )
    threshold_max_value: int = Field(
        default=255,
        ge=0,
        le=255,
        description="Maximum value to use with THRESH_BINARY and THRESH_BINARY_INV",
    )
    threshold_type: Literal[
        "binary", "binary_inv", "trunc", "to_zero", "to_zero_inv"
    ] = Field(default="binary", description="Type of thresholding to apply")
    ...
    min_contour_length: int = Field(
        default=10,
        ge=0,
        description="Minimum length of a detected contour to be considered valid",
    )
    max_contour_length: int = Field(
        default=10000,
        ge=0,
        description="Maximum length of a detected contour to be considered valid",
    )

    # Interface specific settings
    detect_fluid_interface: bool = Field(
        default=True, description="Detect the droplet-fluid interface"
    )
    detect_solid_interface: bool = Field(
        default=True, description="Detect the droplet-solid interface"
    )
    solid_interface_proximity: int = Field(
        default=10,
        ge=0,
        description="Pixels from contact line to search for solid interface",
    )

    @field_validator(
        "gaussian_kernel_size", "sobel_kernel_size", "laplacian_kernel_size"
    )
    @classmethod
