...
class OverlayConfigDialog(QDialog):
    ...
    COLOR_PRESETS = ["red", "green", "blue", "yellow", "magenta", "cyan", "white", "black", "lime"]
    ...
    def _build_ui(self) -> None:
        ...
        form.addRow("Contour color:", self._color_row(self._contour_color_btn, self._contour_color_preview))
        ...
        form.addRow("Point color:", self._color_row(self._point_color_btn, self._point_color_preview))
    ...
    def set_config(self, cfg: Optional[Dict[str, Any]]) -> None:
        if not cfg:
            return
        try:
            self.contour_visible.setChecked(bool(cfg.get("contour_visible", self.DEFAULTS["contour_visible"])))
            try:
                color = QColor(str(cfg.get("contour_color", self.DEFAULTS["contour_color"])))
            except Exception:
                color = QColor("red")
            self._set_contour_color(color)
            self.contour_thickness.setValue(int(cfg.get("contour_thickness", self.DEFAULTS["contour_thickness"])))
            self.contour_dashed.setChecked(bool(cfg.get("contour_dashed", self.DEFAULTS["contour_dashed"])))
            self.contour_dash_length.setValue(int(cfg.get("contour_dash_length", self.DEFAULTS["contour_dash_length"])))
            self.contour_dash_space.setValue(int(cfg.get("contour_dash_space", self.DEFAULTS["contour_dash_space"])))
            self.contour_alpha.setValue(float(cfg.get("contour_alpha", self.DEFAULTS["contour_alpha"])))

            self.points_visible.setChecked(bool(cfg.get("points_visible", self.DEFAULTS["points_visible"])))
            try:
                pcol = QColor(str(cfg.get("point_color", self.DEFAULTS["point_color"])))
            except Exception:
                pcol = QColor("lime")
            self._set_point_color(pcol)
            self.point_radius.setValue(int(cfg.get("point_radius", self.DEFAULTS["point_radius"])))
            self.point_alpha.setValue(float(cfg.get("point_alpha", self.DEFAULTS["point_alpha"])))
        except Exception:
    ...
    def _set_contour_color(self, qcolor: QColor) -> None:
        ...
        self._contour_color_preview.setStyleSheet(f'background-color: {qcolor.name()}; border: 1px solid #222;')

    def _set_point_color(self, qcolor: QColor) -> None:
        self._point_color = qcolor
        self._point_color_preview.setStyleSheet(f'background-color: {qcolor.name()}; border: 1px solid #222;')
