...
def _simple_threshold_fallback(
    img: np.ndarray, settings: EdgeDetectionSettings
) -> np.ndarray:
...
class CannyDetector:
    def detect(self, img: np.ndarray, settings: EdgeDetectionSettings) -> np.ndarray:
        ...
        raw = resolve_plugin_settings(
            "canny", getattr(settings, "plugin_settings", {}), **defaults
        )
...
class ThresholdDetector:
    def detect(self, img: np.ndarray, settings: EdgeDetectionSettings) -> np.ndarray:
        ...
        raw = resolve_plugin_settings(
            "threshold", getattr(settings, "plugin_settings", {}), **defaults
        )
...
class SobelDetector:
    def detect(self, img: np.ndarray, settings: EdgeDetectionSettings) -> np.ndarray:
        ...
        raw = resolve_plugin_settings(
            "sobel", getattr(settings, "plugin_settings", {}), **defaults
        )
...
class ScharrDetector:
    def detect(self, img: np.ndarray, settings: EdgeDetectionSettings) -> np.ndarray:
        ...
        raw = resolve_plugin_settings(
            "scharr", getattr(settings, "plugin_settings", {}), **defaults
        )
        ...
        _, edges = cv2.threshold(
            magnitude, cfg.threshold_value, cfg.max_value, cv2.THRESH_BINARY
        )
        return edges_to_xy(
            edges, settings.min_contour_length, settings.max_contour_length
        )
...
class LaplacianBasicDetector:
    def detect(self, img: np.ndarray, settings: EdgeDetectionSettings) -> np.ndarray:
        ...
        raw = resolve_plugin_settings(
            "laplacian", getattr(settings, "plugin_settings", {}), **defaults
        )
...
class LegacySnakeDetector:
    ...
    def detect(self, img: np.ndarray, settings: EdgeDetectionSettings) -> np.ndarray:
        ...
        raw = resolve_plugin_settings(
            "legacy_snake", getattr(settings, "plugin_settings", {}), **defaults
        )
...
class ThresholdSettings(BaseModel):
    ...
    type: Literal["binary", "binary_inv", "trunc", "to_zero", "to_zero_inv"] = Field(
        "binary"
    )
...
def _zero_crossing_detection(
    ...
    # Apply threshold
    z_c_image[:, :-1] |= ((zero_cross_h & (grad_h > min_gradient)) * 255).astype(
        np.uint8
    )
    z_c_image[:-1, :] |= ((zero_cross_v & (grad_v > min_gradient)) * 255).astype(
        np.uint8
    )
...
class AdaptiveEdgeDetector:
    ...
    def detect(
        ...
        raw_cfg = resolve_plugin_settings(
            "adaptive", getattr(settings, "plugin_settings", {})
        )
...
class LoGEdgeDetector:
    ...
    def detect(
        ...
        # Resolve settings
        raw_cfg = resolve_plugin_settings(
            "log", getattr(settings, "plugin_settings", {})
        )
...
class ImprovedSnakeDetector:
    ...
    def detect(
        ...
        raw = resolve_plugin_settings(
            "improved_snake", getattr(settings, "plugin_settings", {}), **defaults
        )
