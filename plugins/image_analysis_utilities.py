...
def edge_comparison(image: np.ndarray) -> dict:
    ...
    contours_adaptive, _ = cv2.findContours(
        adaptive, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE
    )
    max_length_adaptive = (
        max([len(c) for c in contours_adaptive]) if contours_adaptive else 0
    )
    methods["adaptive"] = {
        "contours": len(contours_adaptive),
        "max_length": max_length_adaptive,
    }
    ...
    results["recommendation_reason"] = (
        f"Longest contour: {methods[best_method]['max_length']} points"
    )
