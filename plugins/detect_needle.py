...
class NeedleSessileSettings(BaseModel):
    ...
    adaptive_block_size: int = Field(
        21, description="Threshold block size (must be odd)"
    )
...
def detect_needle_sessile(
    ...
    **kwargs : dict
    ...
    raw_cfg = resolve_plugin_settings(
        "sessile_needle_detect", kwargs.get("plugin_settings", {}), **kwargs
    )
...
def detect_needle_pendant(
    ...
    **kwargs : dict
    ...
    # Resolve settings
    raw_cfg = resolve_plugin_settings(
        "needle_pendant", kwargs.get("plugin_settings", {}), **kwargs
    )
