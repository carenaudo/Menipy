...
def _binary_to_contour(
    binary: np.ndarray, min_len: int = 0, max_len: int = 100000
) -> np.ndarray:
...
def auto_adaptive_detect(img: np.ndarray, settings=None, **kwargs) -> np.ndarray:
    ...
    # PRIORITY 2: Otsu
        ...
        variance_ratio >= cfg.otsu_variance_threshold
        and illumination_cv <= cfg.illumination_cv_threshold
    ):
        method_used = "Otsu"
    ...
    logger.info(
        f"auto_adaptive_detect: selected {method_used} "
