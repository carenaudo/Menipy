...
"""
    load_and_preprocess(image_path::String; blur_sigma=1.5, threshold=0.5)

Carga y preprocesa la imagen de la gota
"""
function load_and_preprocess(image_path::String; blur_sigma=1.5, threshold=0.5)
    # Cargar imagen
    img = load(image_path)
    ...
    # Binarizar
    img_binary = img_filtered .< threshold  # Nota: < para gota oscura sobre fondo claro
...
function find_substrate_line_robust(contour::Vector{Tuple{Int,Int}}, img_size::Tuple{Int,Int})
    ...
    # Encontrar baseline (puntos más bajos)
    max_y = maximum(y_coords)

    # Tomar puntos en la región inferior (últimos 5% de altura)
    threshold_y = max_y - 0.05 * (maximum(y_coords) - minimum(y_coords))
    base_candidates = filter(p -> p[2] >= threshold_y, contour)

    if length(base_candidates) < 2
        base_candidates = filter(p -> p[2] >= max_y - 10, contour)
    end

    if length(base_candidates) < 2
        return Tuple{Int,Int}[], 0.0, (0, 0), (0, 0), max_y, (0, 0)
    end

    # Puntos de contacto: extremos izquierdo y derecho
    left_contact = base_candidates[argmin([p[1] for p in base_candidates])]
    right_contact = base_candidates[argmax([p[1] for p in base_candidates])]

    # Ajustar línea horizontal en la baseline
    baseline_y = (left_contact[2] + right_contact[2]) / 2

    # Para sustrato inclinado, usar regresión lineal
    x_base = [Float64(p[1]) for p in base_candidates]
    y_base = [Float64(p[2]) for p in base_candidates]

    # Regresión lineal robusta (eliminar outliers)
    if length(x_base) >= 3
        # RANSAC simplificado: usar mediana
        y_median = median(y_base)
        inliers = abs.(y_base .- y_median) .< 5
        x_base = x_base[inliers]
        y_base = y_base[inliers]
    end
    ...
    x_line = range(left_contact[1] - 20, right_contact[1] + 20, length=100)
    substrate_line = [(Int(round(x)), Int(round(slope * x + intercept))) for x in x_line]

    # Encontrar apex (punto más alto)
    apex_idx = argmin(y_coords)
...
function analyze_droplet(image_path::String;
    ...
    if isempty(contour)
        error("❌ No se pudo detectar el contorno de la gota. Ajusta el threshold.")
    end
    ...
    println("   ✓ Puntos de contacto identificados")
    println("   ✓ Apex en: $(apex_point)")
...
println("""
╔═══════════════════════════════════════════════════════════════════╗
║          Script ASDA - Análisis de Gota Sésil v2.0              ║
╚═══════════════════════════════════════════════════════════════════╝

USO:
  1. Configurar backend (si es necesario):
     gr()  # o plotlyjs(), pyplot()
     
  2. Analizar imagen:
     results, img, cleaned = analyze_droplet("ruta/imagen.png",
                                             blur_sigma=1.5,
                                             threshold=0.5,
                                             fit_range=25,
                                             pixel_size=1.0)
...
PARÁMETROS:
  • blur_sigma:  Suavizado (1.0-3.0, default=1.5)
  • threshold:   Umbral binarización (0.0-1.0, default=0.5)
                 Ajusta si la gota no se detecta correctamente
  • fit_range:   Puntos para calcular tangente (15-40, default=25)
  • pixel_size:  Calibración (μm/px o mm/px)

TIPS:
  - Si threshold=0.5 no funciona, prueba valores entre 0.3-0.7
  - Para gotas más oscuras, usa threshold más bajo (0.3-0.4)
  - Para gotas más claras, usa threshold más alto (0.6-0.7)
